...
func (config *Socks5Config) SpawnRoutine(ctx context.Context, vt *VirtualTun) error {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				errorLogger.Printf("SOCKS5 listener goroutine panicked: %v", r)
			}
		}()
		<-ctx.Done()
		listener.Close()
...
func (conf *TCPClientTunnelConfig) SpawnRoutine(ctx context.Context, vt *VirtualTun) error {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				errorLogger.Printf("TCPClient listener goroutine panicked: %v", r)
			}
		}()
		<-ctx.Done()
		server.Close()
...
func (conf *TCPServerTunnelConfig) SpawnRoutine(ctx context.Context, vt *VirtualTun) error {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				errorLogger.Printf("TCPServer listener goroutine panicked: %v", r)
			}
		}()
		<-ctx.Done()
		server.Close()
...
func connForward(from io.ReadWriteCloser, to io.ReadWriteCloser) {
	defer func() {
		if r := recover(); r != nil {
			errorLogger.Printf("connForward panicked: %v", r)
		}
	}()
...
func tcpClientForward(ctx context.Context, vt *VirtualTun, raddr *addressPort, conn net.Conn) {
	defer func() {
		if r := recover(); r != nil {
			errorLogger.Printf("tcpClientForward panicked: %v", r)
		}
	}()
	defer conn.Close()
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				errorLogger.Printf("tcpClientForward copy goroutine 1 panicked: %v", r)
			}
		}()
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				errorLogger.Printf("tcpClientForward copy goroutine 2 panicked: %v", r)
			}
		}()
...
func STDIOTcpForward(ctx context.Context, vt *VirtualTun, raddr *addressPort) {
	defer func() {
		if r := recover(); r != nil {
			errorLogger.Printf("STDIOTcpForward panicked: %v", r)
		}
	}()
	target, err := vt.resolveToAddrPort(raddr)
	if err != nil {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				errorLogger.Printf("STDIOTcpForward copy goroutine 1 panicked: %v", r)
			}
		}()
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				errorLogger.Printf("STDIOTcpForward copy goroutine 2 panicked: %v", r)
			}
		}()
...
func tcpServerForward(ctx context.Context, vt *VirtualTun, raddr *addressPort, conn net.Conn) {
	defer func() {
		if r := recover(); r != nil {
			errorLogger.Printf("tcpServerForward panicked: %v", r)
		}
	}()
	defer conn.Close()
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				errorLogger.Printf("tcpServerForward copy goroutine 1 panicked: %v", r)
			}
		}()
		_, _ = CopyWithPool(conn, sconn)
		done <- struct{}{}
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				errorLogger.Printf("tcpServerForward copy goroutine 2 panicked: %v", r)
			}
		}()
		_, _ = CopyWithPool(sconn, conn)
		done <- struct{}{}
...
func (d VirtualTun) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	defer func() {
		if r := recover(); r != nil {
			errorLogger.Printf("ServeHTTP panicked: %v", r)
...
func (d VirtualTun) pingIPs() {
	defer func() {
		if r := recover(); r != nil {
			errorLogger.Printf("pingIPs panicked: %v", r)
		}
	}()
	for _, addr := range d.Conf.CheckAlive {
		...
		go func() {
			defer func() {
				if r := recover(); r != nil {
					errorLogger.Printf("pingIPs response goroutine panicked: %v", r)
				}
			}()
...
func (d *VirtualTun) StartPingIPs() {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				errorLogger.Printf("StartPingIPs goroutine panicked: %v", r)
			}
		}()
		d.pingIPs()
