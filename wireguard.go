...
// Параметр сохранён для обратной совместимости, но не используется (PingRecord теперь инициализируется в StartPingIPs).
func StartWireguard(conf *DeviceConfig, logLevel int, pingCacheSize int) (*VirtualTun, error) {
	...
	return &VirtualTun{
		...
		PingRecord: nil, // будет создан в StartPingIPs
	}, nil
}
